...
public class HomePlugin : BasePlugin
{
    ...
    {
        _repository =
            new SqliteTeleportRepository($"Data Source={Path.Combine(ModulePath, "teleports.db")};Version=3;");
    ...
    private void OnTriggeredTeleport(ICommandContext ctx)
    {
        ...
        switch (action)
        {
            case "set":
            {
                ...
                if (_repository.GetPointsCount(userId) >= limit)
                {
                    ChatMessenger.SendTo(entityId,
                        $"{tag}{_localization.Translate(userId, "Home limited", limit)}");
                    return;
                }
            ...
            case "remove":
            {
                ...
                {
                    ChatMessenger.SendTo(entityId,
                        $"{tag}{_localization.Translate(userId, "Bad args remove home")}");
                    return;
                }

                var name = ctx.Args[1];

                if (_repository.RemovePoint(userId, name) > 0)
                {
                    ChatMessenger.SendTo(entityId,
                        $"{tag}{_localization.Translate(userId, "Home removed", name)}");
                }
